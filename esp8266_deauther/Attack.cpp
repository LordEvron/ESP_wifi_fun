...
void Attack::run(){
  ...
  /* =============== Deauth Attack =============== */
  if(isRunning[0] && currentMillis-prevTime[0] >= 10){
    ...
    for(int a=0;a<apScan.results;a++){
      if(apScan.isSelected(a)){
        Mac _ap;
        Serial.print("Attacking AP " + String(a));
        ...
        for(int i=0;i<clientScan.results;i++){
          if(clientScan.getClientSelected(i)){
            _selectedClients++;
             Serial.print("ATTACCHING SPECIFIC CLIENT ");
  ...
  /* =============== Beacon clone Attack =============== */
  if(isRunning[1] && currentMillis-prevTime[1] >= 10){
  ...
  /* =============== Beacon list Attack =============== */
  if(isRunning[2] && currentMillis-prevTime[2] >= 10){
  ...
  if(isRunning[3] && currentMillis-prevTime[3] >= 10){
